...
3) Some users have reported difficulties using the yml environment files. The following is an alternative strategy:
   ...
   conda active agcm_environment
   pip3 install torch-harmonics==0.6.3 

4) Edit Preprocess.ipynb variables  
...
5) Run Preprocess.ipynb as a jupyter notebook  
...
6) Edit RunModel.beta.ipynb (or RunModel.PrescribedMean.ipynb) variables  
...
7) Run RunModel.beta.ipynb (or RunModel.PrescribedMean.ipynb)  
...
8) Edit and run a Postprocess script  
...
9) Issues, questions or concerns please contact Ben Kirtman at bkirtman@miami.edu.
