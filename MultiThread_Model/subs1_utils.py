...
def bscst(kmax):
    ...
    # This example below corresponds to the Linear Baroclinic Model (LBM) of
    # Wantanabe and Kimoto (2000; QJRMS). These are the sigma levels
    if kmax == 11:
    ...
    # This example below corresponds the NCAR CAM4 with 26 levels.
    if kmax == 26:
    ...
    # End delsig calculation for the vertical structure.
    ...
    rkappa = 287.05 / 1005.0    # R / Cp; Gas constant divided by specific
    ...
    # 300.0 K below corresponds to a global mean temperature; should not
    # be modified
...
def inv_em(dmtrx, steps_per_day, kmax, mw, zw):
    ...
    dt2 = 2.0 * 86400.0 / steps_per_day  # 86400 is the number of seconds
                                         # per day
    ccc = 0.5 * dt2
    cccs = ccc * ccc
    ae = 6.371E+06 # Radius of the Earth
...
def mcoeff(kmax, si, sl, slkap, r1b, r2b, delsig):
    ...
    # 1005.0 below is the specific heat at constant pressure
    #
    ...
    mu = []
    nu = []
    ### 287.05 Gas constant; 1005.0 Specific heat at constant pressure
    ### 300.0 is global mean background temperature
    #
    [lamda.append(
...
def diffsn(zmn1, zmn3, dmn1, dmn3, tmn1, tmn3, mw, zw):
    ...
    ae = torch.tensor(6.371E+06)  # Radius of the earth
    a4 = torch.pow(ae, 4)
    ### the diffusion coefficients are resolution dependent but are 
    ### internally specified once the spectral truncation is set. 
    ### For divergence, the value of T is set to 21 hours, and for 
    ### temperature and vorticity and moisture, the corresponding 
    ### time is 28 hours.
...
def damp_weakly_prescribed_mean(
    ...
    ### 24*60*60 is the number of seconds per day
    ### 40, 150, 7, 2 & 3 correspond to damping time in days
    ### See BAMS article for discussion of these damping time scales.
...
def damp_heldsuarez(zmn1, zmn3, dmn1, dmn3, tmn1, tmn3, qmn1, qmn3, lnpsclim,
    ...
    ### Dampling parameters below are taken directly from
    ### Held & Suarez (1994; BAMS)
...
def explicit(
    ...
    ae = 6.371E+06  # Radius of the earth
    andree = 2.0e-02 # Time differencing filter parameter; Do not change
    alpha = 0.5
    dt2 = 2.0 * dt
    ccc = alpha * dt2
    cccs = ccc * ccc
   ...
   # 287.05 is gad constant, 300.0 in global mean temperature
...
def potential_temp(temp, sigma, lnps, kmax):
    ...
    gamma = r / cp
    #### 1000.0 is surface pressure in mb
